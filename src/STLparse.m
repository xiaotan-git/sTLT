...
switch(numel(varargin))
    ...
    case 1 % Here, the formula is defined by a string. We parse this string.
        ...
        st = regexprep(varargin{1}, 'true', 'mu_true');
        st = regexprep(st, '^\s*', '');
        st = regexprep(st, 'false', 'mu_false');
        
        % aliases
        st = regexprep(st, 'F\[', 'ev_[');
        st = regexprep(st, 'G\[', 'alw_[');
        
        st = regexprep(st, 'F[^\[]', 'alw');
        st = regexprep(st, 'G[^\[]', 'ev');
        
        st = regexprep(st, 'Vee', 'or');
        st = regexprep(st, 'Wedge', 'and');
        ...
        if success
            phi1 = STLformula([phi.id '_1'],st1);
            phi2 = STLformula([phi.id '_2'],st2);
            phi = STLparse(phi,'or', phi1, phi2);
            phi.level = phi.level+1;
            return
        end
        ...
        if success
            disp('Operator \it{impplies} exists. Not supported!')
        ...
        interval = '[0 inf]';
        if success
            disp('Operator \it{Until} exists. Not supported!')
            phi1 = STLformula([phi.id '1__'],st1);
            phi2 = STLformula([phi.id '2__'],st2);
        ...
        if success&&isempty(st1)
            if regexp(st2,'\<ev|\<alw')
                error('Operator \it{Not} exists. Only negation of a predicate is allowed!')
            end
%             phi1 = STLformula([phi.id '_1'],st2);
%             phi = STLparse(phi, 'not', phi1);
        ...
        [success, st1, st2] = parenthesisly_balanced_split(st, '\<mu');
        if success
            phi.type='predicate';
            phi.st = st;
            return
